...
const CYBERIMPACT_BASE_URL = 'https://api.cyberimpact.com/api';

/** Columns selected from outreach_contacts for this route */
type EligibleContact = Pick<
  OutreachContact,
  'id' | 'contact_email' | 'business_name' | 'pipeline' | 'status' | 'casl_consent_method'
>;
...
async function addMemberToGroup(
  ...
  try {
    const response = await fetch(
      `${CYBERIMPACT_BASE_URL}/groups/${groupId}/members`,
      {
        method: 'POST',
        headers: {
...
export async function POST() {
  ...
  const groupId = process.env.CYBERIMPACT_PSIB_GROUP_ID;
  ...
  /* ── 3. Fetch eligible PSIB contacts ───────────────────────── */
  const { data: contacts, error: contactsError } = await service
    .from('outreach_contacts')
    .select('id, contact_email, business_name, pipeline, status, casl_consent_method')
    .eq('pipeline', 'psib')
    .eq('status', 'prospect')
  ...
  /* ── 4. Fetch PSIB sequence templates (ordered by step) ──── */
  ...
  for (const contact of contacts) {
    ...
    try {
      ...
      if (activityError) {
        console.error(
          `[psib-campaign] Failed to fetch activity for contact ${contact.id}:`,
      ...
      /* 5c. Add contact to Cyberimpact group */
      ...
      if (!result.ok) {
        console.error(
          `[psib-campaign] Cyberimpact error for ${contact.contact_email}:`,
          result.error,
        );
        continue;
      }

      /* 5d. Log activity in outreach_activity_log */
      ...
      if (logError) {
        console.error(
          `[psib-campaign] Failed to log activity for contact ${contact.id}:`,
          logError.message,
        );
        continue;
      }
      ...
      if (updateError) {
        console.error(
          `[psib-campaign] Failed to update status for contact ${contact.id}:`,
          updateError.message,
        );
        continue;
      }
      ...
      const message = err instanceof Error ? err.message : String(err);
      console.error(
        `[psib-campaign] Unexpected error for contact ${contact.id}:`,
        message,
      );
      continue;
    }
